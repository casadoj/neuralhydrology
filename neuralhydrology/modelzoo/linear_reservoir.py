...
class LinearReservoir(BaseConceptualModel):
    ...
    def forward(
        ...
        FFc = torch.tensor(0.1, dtype=torch.float32, device=x_conceptual.device, requires_grad=False) # zero
        ...
        for j in range(x_conceptual.shape[1]):
            ...
            ff = ff + inflow[:, j]
            # linear outflow
            # outflow = ff / torch.mean(parameters['T'][:]) # suggestion from Carlo
            outflow = ff / parameters['T'][:,j]
            # limit outflow so the final storage is between 0 and 1
            outflow = torch.max(torch.min(outflow, ff - FFc), ff - one + eps) # do a test removing this line
            # update storage
            ff = ff - outflow
...
class LinearReservoirV(BaseConceptualModel):
    """Implementation of the linear reservoir model with dynamic parameterization.
    ...
    def __init__(self, cfg: Config):
        super(LinearReservoirV, self).__init__(cfg=cfg)
    ...
    def forward(
        ...
        for j in range(x_conceptual.shape[1]):
            ...
            ff = ff + inflow[:, j]
            # linear outflow
            # outflow = ff / torch.mean(parameters['T'][:,j]) # suggestion from Carlo: mean over the whole batch, i.e., different samples of the same basin
            # outflow = ff / torch.mean(parameters['T'][:,:]) # suggestion from Carlo: mean over the whole batch and warmup period
            outflow = ff / parameters['T'][:,j]
            # limit outflow so the final storage is between FFc and 1
            outflow = torch.max(torch.min(outflow, ff - FFc), ff - one + eps) # do a test removing this line
            # outflow = torch.min(outflow, ff - FFc)
            # update storage
            ff = ff - outflow
...
class LinearReservoirQV(BaseConceptualModel):
    """Implementation of the linear reservoir model with dynamic parameterization.
    ...
    def __init__(self, cfg: Config):
        super(LinearReservoirQV, self).__init__(cfg=cfg)
