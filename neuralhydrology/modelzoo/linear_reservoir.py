...
class LinearReservoir(BaseConceptualModel):
    ...
    def forward(
        ...
        """Performs forward pass on the linear reservoir model. 
        ...
        eps = torch.tensor(1e-8, dtype=torch.float32, device=x_conceptual.device)
        ...
        FFc = zero # this should be different for each reservoir
        ...
        for j in range(x_conceptual.shape[1]):
            ...
            outflow = torch.clamp(outflow, min=ff - zero, max=ff - one + eps)
            # update storage
            ff = ff - outflow
